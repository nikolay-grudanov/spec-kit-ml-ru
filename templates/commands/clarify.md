---
description: Identify underspecified areas in current feature spec by asking up to 5 highly targeted clarification questions and encoding answers back into the spec. Supports both ML and Web/Software projects.
handoffs:
...
2. Load the current spec file. Determine the project type.

   **ML PROJECT INDICATORS**:
   - Spec contains ML Scenarios & Use Cases section
   - Spec contains ML Requirements section
   - Keywords in spec: model, predict, train, accuracy, F1, metrics, classification, regression, neural network, deep learning

   **DETERMINE PROJECT TYPE**:
   - If ML sections present OR 2+ ML keywords found → **ML Project**
   - Otherwise → **Web/Software Project**

3. Perform a structured ambiguity & coverage scan using the APPROPRIATE taxonomy based on project type. For each category, mark status: Clear / Partial / Missing. Produce an internal coverage map used for prioritization (do not output raw map unless no questions will be asked).

   **ML PROJECT TAXONOMY** (use for ML projects):

   ML Model Scope & Behavior:
   - Model type and architecture (CNN, LSTM, tree-based, transformers, etc.)
   ...
   **WEB/SOFTWARE PROJECT TAXONOMY** (use for non-ML projects):
...
4. Generate (internally) a prioritized queue of candidate clarification questions (maximum 5). Do NOT output them all at once. Apply these constraints:
   ...
   **For ML Projects**:
   - Use ML-specific question types (metrics, data quality, validation, security, versioning)
   - Prioritize ML categories: evaluation > data > validation > security > implementation details

   **For Web/Software Projects**:
   - Use general software question types
   - Prioritize: scope > security/privacy > user experience > technical details

   **General Constraints (both project types)**:
...
5. Sequential questioning loop (interactive):
   ...
   - For multiple‑choice questions:
      - **Analyze all options** and determine the **most suitable option** based on:
         - Best practices for the project type (ML vs Web)
...
6. Integration after EACH accepted answer (incremental update approach):
   ...
   - Then immediately apply the clarification to the most appropriate section(s):
      ...
      **For ML Projects**:
      - ML ambiguity → Update or add bullet in ML Requirements (metrics, data, validation, security, versioning)
      - User interaction with model → Update ML Scenarios & Use Cases
      - Model architecture → Update ML Architecture section
      - Data schema/features → Update ML Data Specification
      - Edge case → Add bullet under appropriate ML section
      ...
      **For Web/Software Projects**:
      ...
      - Edge case / negative flow → Add a new bullet under Edge Cases / Error Handling (or create such subsection if the template provides placeholder for it).
      - Terminology conflict → Normalize term across spec; retain original only if necessary by adding `(formerly referred to as "X")` once.

   - If the clarification invalidates an earlier ambiguous statement, replace that statement instead of duplicating; leave no obsolete contradictory text.

   - Save the spec file AFTER each integration to minimize the risk of context loss (atomic overwrite).
