...
scripts:
  sh: python3 .ml-spec/scripts/check_environment.py --json
  ps: python .ml-spec/scripts/check_environment.py --json
---
...
1. **Check ML Environment** (run first):
   Run `.ml-spec/scripts/check_environment.py --json` from repo root **once** to detect current ML environment:
   ```bash
   python3 .ml-spec/scripts/check_environment.py --json
   ```
   
   Parse the JSON output to understand:
   - Python version and environment
   - Available ML libraries (torch, tensorflow, sklearn, etc.)
   - GPU/CUDA availability
   - Development tools (pytest, black, mypy, etc.)
   - Package manager (conda, pip, uv)
   
   Use this context to generate **environment-aware questions**:
   - If GPU detected → Ask about mixed-precision training, batch size optimization
   - If specific libraries missing → Suggest adding them to requirements
   - If CUDA version known → Ask about CUDA-specific optimizations
   
   Example context-aware questions:
   - "I see you have an NVIDIA GPU with CUDA 11.8. Should we configure mixed-precision training (AMP)?"
   - "PyTorch is not installed. Would you like me to add it to requirements.txt for this ML project?"
   - "I notice you're using conda. Should I create an environment.yml instead of requirements.txt?"

2. **Load the current ML spec file** and run `.specify/scripts/get-current-spec.sh` to identify:
   - `FEATURE_DIR`
   - `FEATURE_SPEC`
